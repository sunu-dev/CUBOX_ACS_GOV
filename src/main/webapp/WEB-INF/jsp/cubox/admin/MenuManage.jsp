...
<div class="search_box mb_20">
	<div class="search_in">
		<div class="comm_search mr_20">
			...
			<select name="menu_cl_cd" id="menu_cl_cd" size="1" class="w_190px input_com">
				<option value="">메뉴분류를 선택하세요.</option>
				<c:forEach items="${cboMenuCl}" var="menuCl" varStatus="status">
				<option value='<c:out value="${menuCl.menu_cl_cd}"/>' <c:if test="${params.menu_cl_cd eq menuCl.menu_cl_cd}">selected</c:if>><c:out value="${menuCl.menu_cl_nm}"/></option>
				</c:forEach>
			</select>
		</div>
		<div class="comm_search mr_20">
			<div class="title">메뉴명</div>
			<input type="text" id="menu_nm" name="menu_nm" class="w_150px input_com" value="<c:out value='${params.menu_nm}'/>" placeholder="메뉴명을 입력하세요.">
		</div>
		...
		<table class="tb_list">
			...
			<tbody>
				...
				<tr>
					...
					<td><c:out value="${list.menu_cd}"/></td>
					<td><c:out value="${list.menu_cl_nm}"/><c:if test="${list.cl_use_yn ne 'Y'}"><font class="font-color_H"> (사용안함)</font></c:if></td>
