...
<%
String gvLogGb = System.getenv("FRS_VERIFICATION_LOG_GB");
...
$(function(){ 
	...
	$("#btnExcel").click(function(){
		frmSearch.action = "./veriListXls.do";
		frmSearch.submit();		
	})		
...
function fnDetail(str) {
	...
	$form.attr("target", "winVeriDetailPopup");
	$form.attr("action", "./veriDetailPopup.do");
	$form.appendTo("body");
	
	var vid = $("<input type='hidden' id='srchSn' name='srchSn' value='"+str+"'>");
	$form.append(vid);
	
	openPopup('', 'winVeriDetailPopup', 874, 640);
	$form.submit();
}
</c:if>
</script>
...
<input type="hidden" id="chkValueArray" name="chkValueArray" value="request_dt,result_nm,score,threshold,device_id" /> 
<input type="hidden" id="chkTextArray" name="chkTextArray" value="인증요청일시,인증결과,매칭점수,기준점수,단말기ID">
