...
$(function(){ 
	...
	$("#btnExcel").click(function(){
		frmSearch.action = "./crttGrDayXls.do";
		frmSearch.submit();		
	});
});
...
<input type="hidden" id="chkValueArray" name="chkValueArray" value="stat_de_frm,group_cd,requst_cnt,succes_cnt,succes_rate,failr_cnt,failr_rate" /> 
<input type="hidden" id="chkTextArray" name="chkTextArray" value="요청일자,그룹코드,요청건수,성공건수,성공비율(%),실패건수,실패비율(%)">
		...
		<table class="tb_list3">
			...
			<tbody>
				...
				<tr <c:if test="${list.stat_de eq '합계'}">class="total2"</c:if>>
					<td><c:out value="${list.stat_de_frm}"/></td>
				...
				<c:forEach items="${list}" var="list" varStatus="status">
				<c:choose>
				<c:when test="${list.group_cd eq '합계'}">
				<tr class="total2">
					<td rowspan="<c:out value="${list.cf_cnt}"/>"><c:out value="${list.stat_de_frm}"/></td>
					<td class="bg2"><c:out value="${list.group_cd}"/></td>
					<td class="bg2 td_right"><fmt:formatNumber value="${list.requst_cnt}" type="number"/></td>
					<td class="bg2 td_right"><fmt:formatNumber value="${list.succes_cnt}" type="number"/></td>
					<td class="bg2"><c:out value="${list.succes_rate}"/></td>
					<td class="bg2 td_right"><fmt:formatNumber value="${list.failr_cnt}" type="number"/></td>
					<td class="bg2"><c:out value="${list.failr_rate}"/></td>					
				</tr>
				</c:when>
				<c:when test="${list.group_cd eq '소계'}">
				<tr class="total">
					<td rowspan="<c:out value="${list.cf_cnt}"/>"><c:out value="${list.stat_de_frm}"/></td>
					<td class="bg1"><c:out value="${list.group_cd}"/></td>
					<td class="bg1 td_right"><fmt:formatNumber value="${list.requst_cnt}" type="number"/></td>
					<td class="bg1 td_right"><fmt:formatNumber value="${list.succes_cnt}" type="number"/></td>
					<td class="bg1"><c:out value="${list.succes_rate}"/></td>
					<td class="bg1 td_right"><fmt:formatNumber value="${list.failr_cnt}" type="number"/></td>
					<td class="bg1"><c:out value="${list.failr_rate}"/></td>
				</tr>
				</c:when>
				<c:otherwise>
				<tr>
