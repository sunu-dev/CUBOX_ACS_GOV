...
$(function(){ 
	...
	$("#btnExcel").click(function(){
		frmSearch.action = "./glryDayXls.do";
		frmSearch.submit();		
	});
});
...
<input type="hidden" id="chkValueArray" name="chkValueArray" value="stat_de,regist_cnt,succes_cnt,succes_rate,failr_cnt,failr_rate" /> 
<input type="hidden" id="chkTextArray" name="chkTextArray" value="등록일자,등록건수,성공건수,성공비율(%),실패건수,실패비율(%)">
		...
		<table class="tb_list">
			...
			<tbody>
				...
				<tr <c:if test="${list.stat_de eq '합계'}">class="total2"</c:if>>
					...
					<td class="td_right"><fmt:formatNumber value="${list.regist_cnt}" type="number"/></td>
